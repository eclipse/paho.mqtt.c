...
version.major := $(shell cat version.major)
version.minor := $(shell cat version.minor)
version.patch := $(shell cat version.patch)
...
build: | mkdir $(MQTTLIB_C_TARGET) $(MQTTLIB_A_TARGET) $(MQTTVERSION_TARGET) $(SYNC_SAMPLES) $(ASYNC_SAMPLES) $(SYNC_TESTS)
ifeq ($(WITH_SSL),1)
build: | $(MQTTLIB_CS_TARGET) $(MQTTLIB_AS_TARGET) $(SYNC_SSL_TESTS) $(ASYNC_SSL_TESTS) $(ASYNC_TESTS)
endif
...
ifeq ($(WITH_SSL),1)
$(MQTTVERSION_TARGET): $(MQTTLIB_CS_TARGET)
endif

$(MQTTVERSION_TARGET): $(MQTTLIB_A_TARGET) $(srcdir)/MQTTVersion.c $(srcdir)/MQTTAsync.h
